...
> 🔄 **2026-09-07/08、ステラ取り扱い商品まわりを再設計中**。デイリーカウント入力(①)を廃止し、ステラの営業日(AM4:30締め)ベースの自動計算に置き換える方針が確定・実装中。以下は再設計前の内容（ステラ対象外の商品は変更なし）。詳細・決定事項は`theft-detection-notes.md`の「🔄 再設計メモ」参照。
